...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['tambah'])) {
        ...
        // Set jumlah based on debit/credit
        $jumlah_db = ($jenis_transaksi == 'credit') ? $jumlah : -$jumlah;

        try {
            $conn->beginTransaction();

            // Insert simpanan
            $stmt = $conn->prepare("INSERT INTO simpanan (nasabah_id, jumlah, jenis, tanggal, keterangan) VALUES (?, ?, 'sukarela', ?, ?)");
            ...
            $stmt->execute([$jumlah_db, $nasabah_id]);
                ...
                $conn->commit();
                $success = "Simpanan sukarela berhasil ditambahkan!";
    ...
    if(isset($_POST['edit'])) {
        ...
        try {
            ...
            $stmt->execute([$jumlah_db, $nasabah_id]);
                ...
                $conn->commit();
                $success = "Simpanan sukarela berhasil diupdate!";
                header("Location: " . $_SERVER['PHP_SELF']);
                exit();
            } catch(PDOException $e) {
                $conn->rollBack();
    ...
    if(isset($_POST['hapus'])) {
        ...
        try {
            ...
            $stmt->execute([$id]);
                ...
                $conn->commit();
                $success = "Simpanan sukarela berhasil dihapus!";
                header("Location: " . $_SERVER['PHP_SELF']);
                exit();
            } catch(PDOException $e) {
                $conn->rollBack();
...
$stmt = $conn->query("SELECT s.*, n.nama_lengkap FROM simpanan s JOIN nasabah n ON s.nasabah_id = n.id WHERE s.jenis = 'sukarela' ORDER BY s.tanggal DESC, s.created_at DESC");
...
<?php include '../includes/header.php'; ?>
    ...
    <!-- <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#tambahModal">
        <i class="fas fa-plus"></i> Tambah Simpanan Sukarela
    </button> -->
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <h3 class="card-title">Daftar Simpanan Sukarela</h3>
    ...
    document.querySelectorAll('.delete-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                text: 'Data simpanan sukarela akan dihapus dan saldo nasabah akan dikembalikan!',
                icon: 'warning',
